...
`Vibe Coding Navigator` 不是一个软件工具，而是一套为AI原生开发流程量身定制的`System Prompt`（在Cursor软件中可以作为`Cursor Rules`）。它旨在探索一种全新的软件开发范式：通过设定清晰、严格Prompt来制定一系列规则，使得AI能够实现全程托管，从零开始构建完整项目，实现真正意义上的"无人类代码"开发，同时交付的代码是可控性、可靠性和高质量的。人类开发者可以从繁琐的编码工作中解放出来，转变为项目需求的提出者、开发规则的架构师和最终成果的验证者。
